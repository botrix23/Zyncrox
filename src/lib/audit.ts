/**
 * Sistema de Auditoría Central — Zyncrox
 * Registra eventos críticos de toda la aplicación en la tabla audit_logs.
 *
 * Uso:
 *   import { logAuditEvent } from '@/lib/audit';
 *   await logAuditEvent({ action: 'LOGIN_SUCCESS', userId: user.id, tenantId: user.tenantId });
 */

import { db } from "@/db";
import { auditLogs } from "@/db/schema";

// Catálogo de acciones auditables
export type AuditAction =
  | 'LOGIN_SUCCESS'
  | 'LOGIN_FAILED'
  | 'FORGOT_PASSWORD_REQUESTED'
  | 'LOGOUT'
  | 'TENANT_REGISTERED'
  | 'TENANT_STATUS_CHANGED'   // Activar / Suspender
  | 'TENANT_DELETED'
  | 'IMPERSONATION_STARTED'   // Super admin accede como admin de un tenant
  | 'IMPERSONATION_ENDED'
  | 'SETTINGS_UPDATED'
  | 'APPEARANCE_UPDATED'
  | 'SERVICE_CREATED'
  | 'SERVICE_UPDATED'
  | 'SERVICE_DELETED'
  | 'STAFF_CREATED'
  | 'STAFF_UPDATED'
  | 'STAFF_DELETED'
  | 'BOOKING_CREATED'
  | 'BOOKING_STATUS_CHANGED'
  | 'BOOKING_DELETED'
  | 'BOOKING_ERROR'
  | 'SERVICE_ERROR'
  | 'STAFF_ERROR'
  | 'WOMPI_CREDENTIALS_UPDATED'
  | 'CRON_REMINDERS_RUN'
  | 'CRON_TRIAL_RUN'
  | 'CRON_SURVEYS_RUN'
  | 'ADMIN_CREATED'
  | 'ADMIN_STATUS_CHANGED'
  | 'ADMIN_DELETED'
  | 'TENANT_TRIAL_UPDATED'
  | 'EMAIL_TEMPLATE_UPDATED'
  | 'PLAN_PRICES_UPDATED'
  | 'N1CO_PLAN_CONFIG_UPDATED'
  | 'OWNERSHIP_TRANSFERRED'
  | 'SUPER_ADMIN_RESET_PASSWORD'
  | 'SUPER_ADMIN_RESEND_INVITATION'
  | 'SUPER_ADMIN_DEACTIVATE_USER'
  | 'SUPER_ADMIN_REACTIVATE_USER'
  | 'PRICE_CHANGE_NOTICE_SAVED'
  | 'PRICE_CHANGE_NOTICE_CLEARED'
  | 'PRICE_CHANGE_NOTICE_EMAILS_SENT'
  // Categories
  | 'CATEGORY_CREATED'
  | 'CATEGORY_UPDATED'
  | 'CATEGORY_DELETED'
  // Client notes & contact
  | 'CLIENT_NOTE_CREATED'
  | 'CLIENT_NOTE_UPDATED'
  | 'CLIENT_NOTE_DELETED'
  | 'CLIENT_CONTACT_UPDATED'
  // Loyalty & Points
  | 'LOYALTY_SETTINGS_UPDATED'
  | 'POINTS_SETTINGS_UPDATED'
  | 'REWARD_CREATED'
  | 'REWARD_UPDATED'
  | 'REWARD_DELETED'
  | 'POINTS_REDEEMED'
  // Absences & Blocks
  | 'ABSENCE_REQUESTED'
  | 'ABSENCE_APPROVED'
  | 'ABSENCE_REJECTED'
  | 'BLOCK_CREATED'
  | 'BLOCK_UPDATED'
  | 'BLOCK_CANCELLED'
  // Staff access
  | 'STAFF_ACCESS_CREATED'
  | 'STAFF_ACCESS_REVOKED'
  | 'STAFF_ACCESS_REACTIVATED'
  | 'STAFF_PASSWORD_RESET'
  // Subscription / Billing
  | 'SUBSCRIPTION_ACTIVATED'
  | 'PLAN_CHANGED'
  | 'SUBSCRIPTION_CANCELLED'
  | 'SUBSCRIPTION_REACTIVATED';

export interface AuditEventParams {
  action: AuditAction;
  userId?: string | null;
  tenantId?: string | null;
  details?: Record<string, unknown>;
  ipAddress?: string;
}

/**
 * Escribe un evento en la tabla audit_logs.
 * Never throws — los errores de log no deben interrumpir el flujo principal.
 */
export async function logAuditEvent(params: AuditEventParams): Promise<void> {
  try {
    await db.insert(auditLogs).values({
      action: params.action,
      userId: params.userId ?? null,
      tenantId: params.tenantId ?? null,
      details: params.details ?? null,
      ipAddress: params.ipAddress ?? null,
    });
  } catch (error) {
    // El log de auditoría nunca debe romper el flujo de la aplicación
    console.error('[AuditLog] Error writing audit event:', params.action, error);
  }
}
