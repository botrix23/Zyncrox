            ...
            <tbody className="divide-y divide-slate-100 dark:divide-white/5">
              {sorted.map((row, ri) => (
                <tr key={row.branchId} className="hover:bg-slate-50/60 dark:hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="px-3 py-2.5 text-right font-semibold"><span className={row.cancelled > 0 ? "text-red-500" : "text-slate-300 dark:text-zinc-700"}>{row.cancelled}</span></td>
                  <td className="px-3 py-2.5 text-right"><span className={`inline-block text-xs font-bold px-2 py-0.5 rounded-full ${row.cancellationRate > 20 ? "bg-red-100 dark:bg-red-950/40 text-red-600" : row.cancellationRate > 10 ? "bg-amber-100 dark:bg-amber-950/40 text-amber-600" : "bg-slate-100 dark:bg-white/5 text-slate-500"}`}>{row.cancellationRate}%</span></td>
                  ...
                  <td className="px-3 py-2.5 text-right"><span className="text-sm font-semibold text-blue-600 dark:text-blue-400">{row.newClients}</span></td>
                  <td className="px-3 py-2.5 text-right"><span className="text-sm font-semibold text-slate-600 dark:text-zinc-300">{row.recurringClients}</span></td>
                  <td className="px-3 py-2.5 text-right">
                    {row.occupancyRate !== null ? (
                      <div className="flex items-center justify-end gap-1.5">
                        <div className="w-12 bg-slate-100 dark:bg-white/5 rounded-full h-1.5 overflow-hidden">
                          <div className="h-full rounded-full" style={{ width: `${row.occupancyRate}%`, backgroundColor: row.occupancyRate >= 80 ? "#10b981" : row.occupancyRate >= 50 ? "#9333ea" : "#f59e0b" }} />
                        </div>
