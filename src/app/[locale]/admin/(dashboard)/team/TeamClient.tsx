...
export function TeamClient({ initialAdmins, plan, maxAdmins, isOwner, locale }: TeamClientProps) {
  ...
  const copyPass = (pass: string) => { navigator.clipboard.writeText(pass); setCopied(true); setTimeout(() => setCopied(false), 2000); };
  ...
  const handleDelete = async (admin: Admin) => {
    ...
    setActionId(admin.id); setError(null);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    if (res.success && res.tempPassword) {
      setTempPass(res.tempPassword); setNewName(""); setNewEmail(""); setShowForm(false);
      await refreshAdmins();
    } else {
      ...
      {isPlanLocked ? (
        ...
        <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/5 rounded-3xl overflow-hidden">
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-white/5">
            ...
            {isOwner && canAddAdmin && (
              <button onClick={() => { setShowForm(true); setError(null); }} className="flex items-center gap-2 px-3 py-2 bg-purple-600 hover:bg-purple-500 text-white text-xs font-bold rounded-xl transition-all">
                <Plus className="w-3.5 h-3.5" /> {t("addAdmin")}
              </button>
            )}
          </div>
          {error && <div className="mx-4 mt-4 flex items-center gap-2 p-3 bg-rose-500/10 border border-rose-500/20 rounded-xl text-rose-500 text-xs font-bold"><AlertCircle className="w-4 h-4 shrink-0" /> {error}</div>}
          {tempPass && (
            <div className="mx-4 mt-4 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl p-4 space-y-2">
              ...
              <div className="flex items-center gap-2">
                <code className="flex-1 text-sm font-mono bg-white dark:bg-black/30 px-3 py-2 rounded-xl border border-emerald-500/20 text-slate-900 dark:text-white break-all">{tempPass}</code>
                <button onClick={() => copyPass(tempPass)} className="p-2 bg-emerald-500 hover:bg-emerald-400 text-white rounded-xl transition-all shrink-0">
              ...
              <div key={admin.id} className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-white/5 rounded-2xl">
                ...
                {isOwner && !admin.isOwner && (
                  <div className="flex items-center gap-1 shrink-0">
                    <button onClick={() => handleToggle(admin)} disabled={actionId === admin.id} className="p-2 rounded-xl text-slate-400 hover:text-purple-500 hover:bg-purple-500/10 transition-all disabled:opacity-40">
                      {admin.isActive ? <ToggleRight className="w-4 h-4 text-purple-500" /> : <ToggleLeft className="w-4 h-4" />}
                    </button>
                    <button onClick={() => handleDelete(admin)} disabled={actionId === admin.id} className="p-2 rounded-xl text-slate-400 hover:text-rose-500 hover:bg-rose-500/10 transition-all disabled:opacity-40">
          ...
          {showForm && isOwner && (
            <form onSubmit={handleCreate} className="px-4 pb-4">
              <div className="border-t border-slate-100 dark:border-white/5 pt-4 space-y-2">
                ...
                <input type="email" placeholder={t("emailPlaceholder")} value={newEmail} onChange={e => setNewEmail(e.target.value)} required
                  className="w-full px-4 py-2.5 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 text-sm text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:border-purple-500 transition-colors" />
                <div className="flex gap-2 pt-1">
                  <button type="button" onClick={() => { setShowForm(false); setError(null); }} className="flex-1 py-2.5 rounded-xl text-sm font-bold text-slate-500 hover:bg-slate-100 dark:hover:bg-white/5 transition-all">{t("cancel")}</button>
